...
HAPROXY_TEMPLATE="""# This file autogenerated by jvmctl on {date}
# To regenerate run: jvmctl haproxy_register
...
frontend http 
    bind *:82
    #bind *:443 ssl crt /etc/pki/tls/certs/hammer.nla.gov.au_fullchain.pem

    acl host_haproxy hdr(host) -i haproxy.{hostsuffix}
{acls}
...
@cli_command(group="Hidden")
def haproxy_register(node):
    if not os.path.exists('/etc/haproxy'):
        return

    hostsuffix = socket.gethostname()
    acls = []
    use_backends = []
    backends = []
    ...
    for node in sorted(iter_nodes(), key=lambda node: node.name):
        ...
        acls.append('    acl host_{name} hdr(host) -i {name}.{hostsuffix}'.format(name=node.name, hostsuffix=hostsuffix))
        use_backends.append('    use_backend {name} if host_{name}'.format(name=node.name))
        backends.append(HAPROXY_BACKEND.format(name=node.name, addr=('127.0.0.1:' + port)))
        
    with open('/etc/haproxy/jvmctl.conf', 'w') as f:
        f.write(HAPROXY_TEMPLATE.format(
                date=time.asctime(),
