...
JETTY_PROJECT=jetty-distribution
GIT_BRANCH=HEAD
NLA_ENVIRON=devel
ROOT_URL_PREFIX=/
JAVA_OPTS=
...
def parse_shell_arrays(data):
    ...
    for m in re.finditer(r"(?m)^(?:\s*|^[^=]*=\s*)\(([^)]*)\)\s*$", data):
        body = m.group(1)
        body = body.replace("\n", "\n ")
        out += data[:m.start(1)] + body  # fmt: skip
        pos = m.end()
    out += data[pos:]
    return out
...
class Config(SafeConfigParser):
    def __init__(self, configfile):
        SafeConfigParser.__init__(self, strict=False, allow_no_value=True)
        f = open(configfile)
        try:
            text = f.read()
        finally:
...
JETTY_HTTP_XML_12 = """<?xml version="1.0"?>
<!DOCTYPE Configure PUBLIC "-//Jetty//Configure//EN" "http://www.eclipse.org/jetty/configure_9_0.dtd">
<!-- Auto-generated by jvmctl. Edit {node.config_file} instead -->
<Configure id="Server" class="org.eclipse.jetty.server.Server">
  <Call name="addConnector">
...
class JettyContainer:
    ...
    def __init__(self, node):
        ...
        self.properties = {"jetty.base": self.node.basedir}
    ...
    def configure_jetty8(self):
        ...
        with open(path.join(node.basedir, "context.xml"), "w") as f:
            ...
            for war in self.discover_contexts():
                contexts.append(
                    JETTY_CONTEXT_XML.format(
                        context_path=self.context_path_for_war(war), war=war
                    )
                )
    ...
    def configure_jetty12(self):
        node = self.node
        with open(path.join(node.basedir, "http.xml"), "w") as f:
            f.write(JETTY_HTTP_XML_12.format(node=node))
        with open(path.join(node.basedir, "start.ini"), "w") as f:
            f.write(
                "# Auto-generated by jvmctl. Edit " + node.config_file + " instead\n"
            )
...
class GitRepo(Repo):
    def checkout(self, dest, target):
        ...
        subprocess.check_call(
            ["git", "branch", "-f", "todeploy", "FETCH_HEAD"], env=env
        )
...
class Node:
    ...
    def spawnctl(self, command):
        if (
            command in ["start", "stop", "restart", "enable", "disable"]
            and os.getuid() == 0
            and self.logger
        ):
            self.logger.info(
    ...
    def add_ports_to_firewall(self):
        try:
            ports = f'{self.port()} {unquote(self.config.get("jvm", "EXTRA_FIREWALL_PORTS", fallback=""))}'.split(" ")
            for port in ports:
                if port:
                    print(f"Adding port {port} to firewall ...")
                    subprocess.run(
                        [
    ...
    def pid(self):
        process = subprocess.Popen(
            ["/usr/bin/systemctl", "show", self.svc, "--property=MainPID"],
            stdout=subprocess.PIPE,
        )
    ...
    @property
    def java_home(self):
        result = self.config.get("jvm", "JAVA_HOME", fallback="")
        return "jvm" in result and result or ""

    @property
    def rails_env(self):
        result = self.config.get("jvm", "RAILS_ENV", fallback="")
        return result
...
def delete(node):
    ...
    if path.exists("/usr/sbin/svccfg"):
        subprocess.call(
            ["/usr/sbin/svccfg", "-s", "svc:/site/jetty", "delete", "-f", node.name]
        )
...
def log(node):
    """browse the jvm's log file"""
    logfile = "/logs/%s/stdio.log" % (node.name,)
    if os.path.exists(logfile):
        os.execvp("/usr/bin/less", ["/usr/bin/less", "-R -n +F", logfile])
    # if os.getuid() != 0:
    #     print("Hint: try with sudo")
    if "SYSTEMD_PAGER" not in os.environ:
        os.environ["SYSTEMD_PAGER"] = "LESS=FRXM less"
    os.execvp("/usr/bin/sudo", ["/usr/bin/journalctl", "-u", node.svc] + sys.argv[3:])
...
@cli_command(group="Configuration")
def new(node):
    """Instantiate a new jvm. Run jvmctl config afterward to configure it."""
    if path.exists(node.config_file):
...
@cli_command(group="Configuration")
def view(node, *args):
    """show the jvm application's configuration using 'less'."""
    show(node, "-l")
...
def show(node, *args):
    ...
    try:
        ...
        print(
            f"{C_Red}Error{C_Default}: '{cmd}' command not found. Please make sure it is installed."
        )
    except subprocess.CalledProcessError:
        print(
            f"{C_Red}Error{C_Default}: Could not open or display '{node.config_file}' using '{cmd}'."
        )
...
def config(node):
    ...
    try:
        subprocess.check_call(
            ["git", "rev-parse", "--is-inside-work-tree"], stdout=open(os.devnull, "wb")
        )
    except:
    ...
    if subprocess.call(["git", "diff-index", "--quiet", "HEAD"]):
        ...
        subprocess.check_call(
            [
                "git",
                "commit",
                '--author="{0} <{0}@nla.gov.au>"'.format(os.getlogin()),
                '-m "Config change for {}"'.format(node.name),
            ]
        )
    return result
...
def changed(node):
    ...
    os.chdir(CONF_ROOT)
    _hash = fetch_hash(node)
    subprocess.check_call(["git", "show", _hash])
...
def revert(node):
    ...
    _hash = fetch_hash(node)
    subprocess.check_call(["git", "revert", _hash, "--no-edit"])
...
def stack(node):
    ...
    _pid = node.pid()
    stat = os.stat("/proc/%d" % _pid)
    jstack = path.join(node.java_home, "bin/jstack")
    return subprocess.call(
        [jstack, str(_pid)], preexec_fn=switchuid(stat.st_uid, stat.st_gid)
    )
...
def gcutil(node):
    ...
    return subprocess.call(
        [jstat, "-gcutil", str(_pid)] + sys.argv[3:],
...
def build(node, workarea, args):
    ...
    if node.java_home:
        os.environ["PATH"] = node.java_home + "/bin:/usr/local/bin:/bin:/usr/bin"
...
def deploy(node, *args):
    # The indent in the document stanza is so it looks good when usage() is shown.
    # fmt: off
    """(re)build and (re)deploy the application.
              To deploy an app, JAVA_HOME or RAILS_ENV must be present in the config. If not, then the app is restarted."""
    # fmt: on
    node.ensure_valid()
...
@cli_command(group="Hidden")
def oomkill(node, _pid):
    try_kill_jvm(node, _pid)
    try_rename_heap_dump(node)
    send_oom_email(node, _pid)


def send_oom_email(node, _pid):
...
def try_kill_jvm(node, _pid):
    print("jvmctl oomkill", node.name, _pid)
    try:
...
def post_config(node):
    properties = {"jvmctl.node": node.name}
    property_opts = (
        fmt_properties(properties) + " " + fmt_properties(node.container.properties)
    )
    jvm_opts = [
        "-Xmx" + node.config.get("jvm", "HEAP_SIZE"),
    ...
    set_unless_present(
        node.config, "systemd.service.Service", "WorkingDirectory", node.basedir
    )
    set_unless_present(
        node.config, "systemd.service.Service", "EnvironmentFile", env_file
    )
    set_unless_present(
        node.config, "systemd.service.Service", "User", node.config.get("jvm", "USER")
    )
    set_unless_present(
        node.config, "systemd.service.Service", "ExecStart", cmd.replace("%", "%%%%")
    )
...
def systemd_register(node):
    ...
    with os.fdopen(
        os.open(env_file, os.O_WRONLY | os.O_CREAT | os.O_TRUNC, 0o600), "w"
    ) as f:
    ...
    with open("/etc/systemd/system/" + node.svc + ".service", "w") as f:
        ...
        for section in node.config.sections():
            if not section.startswith("systemd.service."):
                continue
...
def fetch_hash(node):
    ...
    try:
        ...
        die(
            "{} was not found in version control.  Try editing the config with \n jvmctl config {}".format(
                node.config_file, node.name
            )
        )
    if not _hash:
...
def check_conf_exists(args, files):
    """Check list of passed in application names and see if there is a corresponding .conf file"""
    # res = ["test"]
    # print("check_conf_exists", type(args), args)
    res = []
    for fname in args:
        if f"{fname}.conf" in files:
            res.append(Node(fname))
            # print("check_conf_exists3", type(args), args, res)
    return res
...
def list(*apps):
    ...
    # fmt: off
    """list applications
              If no parameters added, then list all applications.
              If application names are listed, then only list those applications, if they are valid.
              EG:
                jvmctl list pywb pandas Pandas odin
                will only list pywb, pandas and odin. Pandas will be ignored as it is the wrong case."""
    # fmt: on
    # param: *args the list of apps
    #         args[1] is a Node class of the first application on the command line.
    #         the rest will be text node names.
    if len(apps) == 1:
        # As the first param is always a node, we have to check for that
        apps[0].ensure_valid()
        nodes = [apps[0]]
    elif len(apps) == 0:
        # If nothing passed in, we will list them all
        nodes = sorted(iter_nodes(), key=lambda node: node.name)
    else:
        # go through all the list and add the first node back into the result
        files = os.listdir(CONF_ROOT)
        nodes = check_conf_exists(apps[1:], files)
        nodes.insert(0, apps[0])
    print(
        "%-30s  %7s  %5s  %-10s %-33s %s"
        % ("NODE", "VERSION", "PORT", "USER", "JAVA_HOME/RAILS_ENV", "STATUS")
    )
    for node in nodes:
        _pid = node.pid()
        if os.path.exists(
            "/etc/systemd/system/multi-user.target.wants/" + node.svc + ".service"
        ):
            if _pid:
                _status = "running as " + str(_pid)
            else:
                _status = "stopped"
        else:
            _status = "disabled"
        ...
        print(
            ...
            % (
                ...
                node.port() or "-",
                node.user or "-",
                node.java_home or node.rails_env or "-",
                _status,
            )
        )
...
def usage():
    print(f"{F_UNDERLINED}Control and deploy JVM applications{C_Default}")
    print(f"Usage: {sys.argv[0]} <app> <command> [parameters]")
    print(f"   or: {sys.argv[0]} <command> <app> [parameters]")
    print("parameters, if used, must be last.")
...
def main():
    if len(sys.argv) == 2:
        ...
        arg2 = sys.argv[2]
        args = sys.argv[3:]
        # arg1 and arg2 can be reversed. eg show pyweb or pyweb show
        if arg1 in commands:
            sys.exit(commands[arg1](Node(arg2), *args) or 0)
        elif arg2 in commands:
            sys.exit(commands[arg2](Node(arg1), *args) or 0)
        else:
            print(f"Unknown command: {arg1} {arg2}", file=sys.stderr)
            usage()
